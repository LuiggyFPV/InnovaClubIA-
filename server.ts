...
app.post("/api/analyze", rateLimiter, async (req, res) => {
  try {
    const estado = sanitizeString(req.body.estado);
    const sector = sanitizeString(req.body.sector);
    const modelo = sanitizeString(req.body.modelo);
    const presupuesto = sanitizeString(req.body.presupuesto);
    const publico = sanitizeString(req.body.publico);
    const diferenciacion = sanitizeString(req.body.diferenciacion);
    const desc = sanitizeString(req.body.desc, MAX_DESC_LENGTH);
    const inst = sanitizeString(req.body.inst, MAX_DESC_LENGTH);
...
app.post("/api/scan", rateLimiter, async (req, res) => {
  try {
    const sector = sanitizeString(req.body.sector);
    const ubi = sanitizeString(req.body.ubi);
    const range = sanitizeString(req.body.range, 10);
...

{
  ...
  "recomendacion": "Estrategia de localización definitiva para penetrar esta zona de forma exitosa frente al radar obtenido."
    ...
    res.status(500).json({ error: "Error al escanear radar geográfico." });
  }
});

// 3. API: Innovatorio Project Postulation & Admission Evaluation
app.post("/api/evaluate-project", rateLimiter, async (req, res) => {
  try {
    const pInovacion = sanitizeString(req.body.pInovacion, 50);
    const pTecnologia = sanitizeString(req.body.pTecnologia, 50);
    const pEscalabilidad = sanitizeString(req.body.pEscalabilidad, 50);
    const pEquipo = sanitizeString(req.body.pEquipo, 50);
    const pPrototipo = sanitizeString(req.body.pPrototipo, 50);
    const pMercado = sanitizeString(req.body.pMercado, 50);
    const pIngresos = sanitizeString(req.body.pIngresos, 50);
    const pPropiedad = sanitizeString(req.body.pPropiedad, 50);
    const pESG = sanitizeString(req.body.pESG, 50);
    const pTraccion = sanitizeString(req.body.pTraccion, 50);
    const pAdquisicion = sanitizeString(req.body.pAdquisicion, 50);
    const pFinanciamiento = sanitizeString(req.body.pFinanciamiento, 50);
    const pNombre = sanitizeString(req.body.pNombre);
    const pNombreProyecto = sanitizeString(req.body.pNombreProyecto);
    const pDescripcion = sanitizeString(req.body.pDescripcion, MAX_DESC_LENGTH);
...
Nota de calificación: Calcula el 'porcentajeAvance' de manera ponderada con base en los parámetros para que sea un reflejo real de la madurez del proyecto. Si es >= 80, pon aprobado en true, de lo contrario false, pero igual entrégales el plan de acompañamiento de Innovaclub para ayudarlos a llegar al 100%.`;
    ...
    res.status(500).json({ error: "Error al evaluar la postulación del proyecto." });
  }
});
