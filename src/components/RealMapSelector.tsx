...
export default function RealMapSelector({ onLocationSelected, currentUbi }: RealMapSelectorProps) {
  ...
  const reverseGeocode = async (lat: number, lng: number) => {
    try {
      ...
      console.warn("Reverse geocode failed, falling back to raw coordinates:", e);
  ...
  const handleSearchAddress = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        console.error("Nominatim search returned status:", response.status);
      ...
      alert("Error de conexión al buscar la ubicación. Verifica tu conexión a internet.");
    } finally {
